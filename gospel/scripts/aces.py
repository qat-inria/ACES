...
from gospel.noise_models.uniform_two_qubit_depolarising_noise_model import (
    UniformTwoQubitDepolarisingNoiseModel,
)
...
def generate_plot(
    ...
    if vline != 0:
        ...
        #plt.xlabel(r"$100 \times |\hat{\lambda} - \lambda_{\rm th}|$", fontsize=12)
    ...
    stats_text = (
        ...
        f"Std: {np.std(inferred_lambdas):.5e}"
    )
    plt.text(
        0.75,
        0.95,
...
def cli(
    ...
    #noise_model = UniformTwoQubitDepolarisingNoiseModel(entanglement_error_prob=depol_prob)
    #noise_model = UniformTwoQubitDepolarisingNoiseModel(
    #    entanglement_error_prob=depol_prob,
    #    parametrization="two_qubit",
    #)
    ...
    #lambda_expected = 1 - 16 / 15 * depol_prob
    ...
    lambda_diff_abs_pct: list[float] = [abs(d) * 100 for d in lambda_diff]
    ...
    generate_plot(
        ...
        #list(inferred_lambdas), vline=1 - 16 * depol_prob / 15
    )  # typing to list necessary?

    generate_plot(
        lambda_diff_abs_pct,
        vline=0,
        target=Path("plot_diff_abs.png")
    )
