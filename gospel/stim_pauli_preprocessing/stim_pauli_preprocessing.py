...
def pattern_to_stim_circuit(
    ...
    for cmd in actual_pattern:
        ...
        #            circuit.append("DEPOLARIZE2", [q0, q1], prob)
            ...
            match cmd.noise:
                case DepolarisingNoise(prob=prob):
                    # Support both 1Q and 2Q uses of DepolarisingNoise
                    if len(cmd.nodes) == 1:
                        (q,) = cmd.nodes
                        circuit.append("DEPOLARIZE1", [q], prob)
                    elif len(cmd.nodes) == 2:
                        q0, q1 = cmd.nodes
                        # Uniform 2Q depol: (1-p)ρ + (p/15) Σ_{P≠II} PρP
                        circuit.append("DEPOLARIZE2", [q0, q1], prob)
                    else:
                        raise ValueError(
                            f"DepolarisingNoise with unsupported arity "
                            f"{len(cmd.nodes)} on nodes={cmd.nodes}"
                        )
                case TwoQubitDepolarisingNoise(prob=prob):
                    q0, q1 = cmd.nodes
                    circuit.append("DEPOLARIZE2", [q0, q1], prob)
